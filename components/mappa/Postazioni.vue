...
console.log(
  "categoria: " +
    postazioniStore.postazioni[selezionato.value as any].id_categoria
);
...
function selezionaCategoria(id: String) {
  ...
  for (const element of postazioniStore.postazioni) {
    ...
    if (element.id_categoria == id) {
      //console.log("Elemento trovato:", "cellaSVG");
      if (cellaSVG) cellaSVG.style.fill = "rgba(54, 196, 252, 0.1)";
    } else {
      console.log("gh'è no");
    }
  }
...
function selezionaDisabilitate() {
  ...
  for (const element of postazioniStore.postazioni) {
    ...
    if (element.stato == 1) {
      ...
      if (cellaSVG) cellaSVG.style.fill = "rgba(255, 255, 0, 0.5)";
    } else {
      console.log("gh'è no");
    }
  }
  ...
  <div style="width: fit-content; width: 100%">
    ...
    <svg class="map-overlay" viewBox="0 0 958 548" id="svgmap">
      ...
      <rect
        style=""
        id="1"
        d="M 10 10 H 190 V 80 A 20 20 0 0 1 170 100 H 10 Z"
        x="2"
        y="143"
        width="107"
        height="151"
    ...
    <PopupElement
      ...
      :key="0"
      style="position: absolute; top: 0px; left: 0px; scale: 2"
      id="popup"
      v-if="mostra == true"
    >
